...
| 영역 | 내용 |
|---|---|
| 스토리 | 이슈 7개 · 보스는 `B` 미션에서만 등장 · 이슈 절반(보스 2명 포함) 클리어 시 다음 이슈 해금 |
| 보스 | 34종. 이슈 1~6은 계열별 팔레트 스왑 + 동일 패턴(원작 방식) |
| 에이지 오브 울트론 | 오리지널 이슈 7. 보스 4기가 각각 다른 모델·패턴이고 순서대로 강해진다. 잡몹은 울트론 센트리 |
| 구간 | 옥상 달리기 · 웹 스윙 · 벽 타기 · 자유 낙하가 끊김 없이 이어짐, 구간별 시점 전환 |
| 적 | 일반 / 무장(투사체) / 장갑(슬라이드 전용) / 비행(점프 전용) / 미니언(미스테리오전) / 센트리(울트론전) |
| 카드 | 65종 · 커먼(3★)~타이탄(8★) · 등급 특성 + 캐릭터 고유 능력 18종 + 타이탄 전용 능력 3종 |
| 팀 | 컬렉션 탭 상단에서 편성 · 슬롯 3칸 · 드래그로 배치/순서 변경 · 가장 앞선 슬롯이 리더 · 빈 팀으로는 출격 불가 |
