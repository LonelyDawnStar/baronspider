...
function hitPlayer(damage=1){
  if(R.inv>0||R.dead||S.god)return; if(R.shields>0){ R.shields=Math.max(0,R.shields-damage); R.inv=1.5; R.combo=Math.floor(R.combo/2); impact(0.5,proj(R.px,0.8,0).x,proj(0,0.8,0).y,'#21c6de'); R.msg=damage>1?'쉴드 전부 소진!':'SHIELD!'; R.msgT=0.8; SFX.play('bomb'); return; }
...
function doJump(){if(!R||R.dead||R.paused)return;if(R.seg==='swing'||R.seg==='wall'||R.seg==='fall')return;
  if(R.state==='run'){ R.state='jump'; R.vy=7.2; SFX.play('jump');
    ...
    for(const o of R.objs){
      if(o.hit||o.passed||o.lane!==R.lane)continue;
      const target=(o.type==='bomb'&&R.boss&&R.boss.phase==='fight')||
        (o.type==='enemy'&&['std','armed','fly','minion'].includes(o.kind));
      const dz=o.z-pz;
      if(target&&dz>1.5&&dz<11*R.mods.zipRange&&dz<bd){best=o;bd=dz;}
    }
...
function updateTitan(dt){ const T=R.mods.titan; if(!T)return; const pz=R.dist/2.2;
  if(T==='uni'){ if(R.uniT>0){ R.uniT-=dt; R.inv=Math.max(R.inv,0.2); if(Math.random()<0.5)R.parts.push({x:proj(R.px,0,0).x+rnd(-50,50),y:proj(0,0.6,0).y+rnd(-60,60),t:0.4,c:'#ffd23a',dust:true,vx:rnd(-40,40)});
      ...
      for(const o of R.objs){ if(o.hit||o.passed)continue; const dz=o.z-pz; if(dz>=0.7||dz<-0.5||o.lane!==L)continue;
        if(o.type==='vial'){o.hit=true;R.vials++;R.score+=10;} else if(o.type==='ring'){o.hit=true;addCombo(1);} else if(o.type==='enemy'){ const k=o.kind; const kill=(k==='armor')?R.state==='slide':(k==='fly')?R.state==='jump':(R.state==='slide'||(R.state==='jump'&&(!R.zip||R.zip.o!==o))); if(kill)killObj(o,dz,'CLONE!','#21c6de'); else {c.alive=false;c.t=15;o.hit=true;R.msg='분신 소멸';R.msgT=0.7;} }
...
function update(dt){
  ...
  if(R.boss){ const b=R.boss; b.bob+=dt; b.hurtT=Math.max(0,b.hurtT-dt); b.decoyT=Math.max(0,b.decoyT-dt); b.x+=(b.tx-b.x)*dt*2.2;
    ...
    else if(b.phase==='fight'){ b.t-=dt; $('#hBossT').style.width=(b.t/b.tmax*100)+'%'; $('#hBossLbl2').textContent='도주 타이머'; $('#hBossT3').style.width='0%'; b.atk-=dt; if(b.atk<=0&&!bossPatternBusy(b)){ bossPattern(b); if(b.rng()<0.6)VOICE.boss(b.fam,'attack'); b.tx=(Math.floor(b.rng()*3)-1)*0.7; b.atk=Math.max(1.0,2.4+b.rng()*1.4-Math.min(1.2,R.issue*0.18)-(b.lvl||0)*0.18); }
  ...
  for(const o of R.objs){
    ...
    if(dz<0.7&&dz>-0.5){
      if(o.type==='vial'){ const near=same||(R.mods.magnet&&Math.abs(R.px-(o.lane-1))<1.3)||R.uniT>0; if(near&&(R.seg!=='swing'||Math.abs((o.y||0)-R.py)<1.2)){o.hit=true;R.vials++;R.score+=10;SFX.play('vial',R.vials); if(R.vials%5===0){addCombo(1);R.msg='VIAL STREAK';R.msgT=0.4;} } }
      ...
      else if(o.type==='bomb'){ if(canStrikeShield(o,same))strikeShield(o); }
          ...
          if(k==='std'||k==='armed'||k==='minion'||k==='sentry')kill=R.state==='jump'||R.state==='slide'; else if(k==='armor'){kill=R.state==='slide';block=R.state==='jump';} else if(k==='fly'){kill=R.state==='jump'||(R.mods.slideFly&&R.state==='slide');}
          if(kill){SFX.play('kill');o.hit=true;R.enemies++; if(R.mods.killCombo)addCombo(R.mods.killCombo); { const ip=proj(o.lane-1,0.8,Math.max(0,dz)); impact(0.35,ip.x,ip.y,'#ffd23a'); ragdoll(o,dz); } R.score+=100*(1+teamMult(4)*0.1)*R.mods.killMul;addCombo(1);R.parts.push({x:proj((o.lane-1),0.8,0).x,y:proj(0,0.8,0).y,t:0.4,c:'#ff5a5f'});} else {o.hit=true;hitPlayer();} } }
  ...
  // Patch 0.1: numeric HUD at 10 Hz; simulation and input remain frame-driven.
  if(R.t<(R.nextHudT||0))return;
  R.nextHudT=R.t+0.1;
  // HUD
...
function draw(){
  ...
  ctx.save(); ctx.translate(sx,sy); R.zoomPunch=Math.max(0,(R.zoomPunch||0)-dtLast*0.5); R.zoom=lerp(R.zoom||1,((R.boss&&R.boss.phase==='beat')?1.6:1)+(R.zoomPunch||0),Math.min(1,dtLast*6)); if(R.shake>0&&R.shakeRot){ ctx.translate(W/2,H/2); ctx.rotate(R.shakeRot*R.shake*Math.sin(R.t*60)); ctx.translate(-W/2,-H/2); } if(R.zoom>1.001){ const bp=proj(R.boss?R.boss.x:0,0.9,14); const cx=lerp(W/2,bp.x,0.6), cy=lerp(H*0.62,bp.y,0.7); ctx.translate(cx,cy); ctx.scale(R.zoom,R.zoom); ctx.translate(-cx,-cy); }
  ...
  if(R.boss){ const b=R.boss; const draws=[[b.x,1]]; if(b.fam==='mysterio'&&b.decoyT>0){draws.push([b.x-1.3,0.45],[b.x+1.3,0.45]);}
    for(const [bx,al] of draws){ const p=proj(bx,0.5,14); ctx.save(); ctx.globalAlpha=al; ctx.translate(p.x,p.y); if(b.phase==='finish'){ const k=Math.min(1,b.finT/1.1); const e=k*k; ctx.translate(0,-e*H*0.9); ctx.rotate(e*Math.PI*4); ctx.scale(1-e*0.8,1-e*0.8); if(b.finT<0.1){ctx.globalAlpha*=0.75;} } ctx.fillStyle='#00000066'; ctx.beginPath(); ctx.ellipse(0,6,70*p.s*2,10*p.s*2,0,0,7); ctx.fill(); drawBossModel(b,bossModelScale(b,p.s),R.t); ctx.restore(); }
    if(b.phase!=='finish'){ // 컴팩트 게이지: 보스 머리 위
      const p=bossHudAnchor(b); const w=150, hh=6; ctx.save(); ctx.translate(p.x,p.y); const disp=canvasFont('--disp');
      ...
      if(b.phase==='beat'){ bar(hh+5,1-b.taps/b.needTaps,'#21c6de'); ctx.fillStyle='#ffd23a'; ctx.fillRect(-w/2,hh*2+10,w*clamp(b.tapT/b.tapMax,0,1),2); ctx.font=`700 11px ${canvasFont('--mono')}`; ctx.fillStyle='#fff'; ctx.strokeText(`연타 ${b.taps}/${b.needTaps}`,0,hh*2+26); ctx.fillText(`연타 ${b.taps}/${b.needTaps}`,0,hh*2+26); }
      else if(b.phase==='intro'){ ctx.fillStyle='#fff'; ctx.font=`700 11px ${canvasFont('--mono')}`; ctx.strokeText('등장 중 — 무적',0,hh+18); ctx.fillText('등장 중 — 무적',0,hh+18); }
      else { bar(hh+5,b.t/b.tmax,'#21c6de'); }
      ctx.restore(); }
    if(R.bubble&&R.bubble.t>0){ R.bubble.t-=dtLast; const bb=R.bubble; const hp=bossHudAnchor(b); const p={x:hp.x+160,y:hp.y-24}; const k=Math.min(1,(2.4-bb.t)/0.15); ctx.save(); ctx.translate(p.x,p.y); ctx.scale(k,k); ctx.font=`700 15px ${canvasFont('--body')}`; const tw=Math.min(300,ctx.measureText(bb.txt).width+26); const lines=[]; { let s=bb.txt,cur=''; for(const ch of s){ if(ctx.measureText(cur+ch).width>tw-24){lines.push(cur);cur=ch;} else cur+=ch; } lines.push(cur); } const th=lines.length*20+14; const spiky=bb.kind==='out'||bb.kind==='beat';
...
function shieldImpact(b){
  b.shieldHitAt=R.t;
  // Feedback budget: one short burst per 120ms even when several bombs arrive.
  if(R.t>=(b.nextShieldSfxT||0)){
    b.nextShieldSfxT=R.t+0.12;
    const p=proj(b.x,0.5,14), y=p.y-105*bossModelScale(b,p.s);
    R.parts.push({x:p.x,y,t:0.25,c:'#2fd3e6',big:true});
    for(let i=0;i<6;i++)R.parts.push({burst:true,x:p.x,y,a:i*Math.PI/3+0.2,len:26+(i%2)*14,t:0.22,c:i%2?'#ffd780':'#bffaff'});
    SFX.sample('hit',{g:0.95,rate:0.72,verb:0});
  }
  if(b.hp<=0)VOICE.boss(b.fam,'beat');
}
function impact(str,x,y,col='#fff'){ // 타격감 패키지: 히트스톱 + 화면 흔들림 + 줌 펀치 + 임팩트 라인
  R.hitstop=Math.max(R.hitstop||0,Math.min(0.045,str*0.025)); R.shake=Math.max(R.shake,0.15+str*0.5); R.zoomPunch=Math.max(R.zoomPunch||0,0.02+str*0.06); R.shakeRot=(Math.random()-0.5)*str*0.06;
...
$('#bSettings').onclick=()=>{
 const d=$('#dlg');d.innerHTML=`<div class="dlg"><h3>설정 · 0.15</h3>
